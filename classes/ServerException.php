...
// Class for unknown MIME extension exception.  Thrown when we don't know the
// file extension for a given MIME type.  This generally means that all files
// are accepted since if we have a list of known MIMEs then they have
// extensions coupled to them.
...
// Class for unknown extension MIME type exception.  Inverse of 
// UnknownMimeExtension - error for when we know the extension but not the MIME 
// type.
...
// Class for a server exception caused by looking in the local filesystem for
// something we store remotely.
...
// given transport.  This likely is either a misconfiguration, or using a 
// queue handler that doesn't implement a transport the site is actively using.
...
// Class for a server exception caused by a notice not having a parent. This
// happens a lot, since many notices are not part of an existing convo, so I
// have put it in LOG_DEBUG severity level.
...
// Class for a server exception caused by a notice having no given type; most
// often this will happen because of an unrecognized activity verb. Since this
// is not neccesarialy an error, but rather a federation incompatability,
// I have put it in LOG_WARNING severity.
...
// Class for a server exception caused by not finding a route map to the given
// location.  This happens in normal operation with 404 errors but can also
// happen with malfunctioning or misprogrammed plugins, so I have given it
// LOG_DEBUG severity.
...
// attempting to request, but postActiv does not currently support it.  This is
// caused internally by malfunctioning plugins, usually, so I have assigned it
// LOG_WARNING severity.
...
// Class for an exception when a URL is invalid.  I put this in LOG_INFO since 
// it can be useful to find federation errors in normal operation.
...
// Class for a server exception caused by password hashing to fail.  Since this 
// compromises the security of client accounts, I have assigned this 
...
// Exception indicating we've got a remote reference to a local user, not a 
// remote user!
//
// If we can ue a local profile after all, it's available as $e->profile.
// -mmn
//
// Most of the time this can happen entirely innocently, especially with older
// versions of GNU social or StatusNet, but it is worth noting, so I have 
// assigned it LOG_INFO severity. -mb
class OStatusShadowException extends Exception {
   public $profile;
   function __construct(Profile $profile, $message) {
      $this->profile = $profile;
      parent::__construct($message, SERVER_EXCEPTION_OSTATUS_SHADOW_FOUND, null);
   }
}
