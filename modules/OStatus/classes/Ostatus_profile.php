 ...
 * OStatus specific profile stores.
 *
 * I'm not entirely clear on why we need a store specific for OStatus since it
 * is the native protocol and ActivityPub builds on it, so this could be a
 * candidate for merging into the normal profile - mb
 *
 * PHP version:
 * Tested with PHP 7
 * ----------------------------------------------------------------------------
...
// Superclass containing the OStatus profile as it is stored in the back-end
// database, as well as the related interfaces.
//
// Properties:
// o __table = 'ostatus_profile'
// o uri
// o profile_id
// o group_id
// o peopletag_id
// o feeduri
// o salmonuri
// o avatar - remote URL of the last avatar we saved
// o created
// o modified
class Ostatus_profile extends Managed_DataObject {
   ...
   //
   // Returns:
   // o array array of column definitions
   ...
   //
   // Error States:
   // o throws NoProfileException if it was not found
   ...
   // Returns an XML string fragment with profile information as an Activity 
   // Streams noun object with the given element type.
   //
   // Assumes that 'activity' namespace has been previously defined.
   //
   // FIXME: 
   // o Replace with wrappers on asActivityObject when it's got everything.
   //
   // Parameters:
   ...
   // -------------------------------------------------------------------------
   // Fuhnction: subscribe
   // Send a subscription request to the hub for this feed. The hub will later 
   // send us a confirmation POST to /main/push/callback.
   //
   // Returns:
   // o void
   //
   // Error States:
   // o throws ServerException if feed state is not valid or subscription fails.
   ...
   // Check if this remote profile has any active local subscriptions, and if 
   // not drop the WebSub subscription feed.
   //
   // Returns:
   // o boolean true if subscription is removed, false if there are still 
   // subscribers to the feed
   // 
   // Error States:
   // o throws Exception of various kinds on failure.
   ...
   //
   // Returns:
   // o boolean true if subscription is removed, false if there are still subscribers to the feed
   //
   // Error States:
   // o throws Exception of various kinds on failure.
   ...
   //
   // Error States:
   // o throws NoProfileException if there is no local profile for the object
   ...
   //
   // Parameters:
   // o Profile $actor  Actor who did the activity
   // o string  $verb   Activity::SUBSCRIBE or Activity::JOIN
   // o Object  $object object of the action; must define asActivityNoun($tag)
   ...
   // Send a Salmon notification ping immediately, and confirm that we got an 
   // acceptable response from the remote site.
   //
   // Parameters:
   // o mixed $entry XML string, Notice, or Activity
   // o Profile $actor
   //
   // Returns:
   // o boolean success
   ...
   // Queue a Salmon notification for later. If queues are disabled we'll send 
   // immediately but won't get the return value.
   //
   // Parameters:
   // o mixed $entry XML string, Notice, or Activity
   //
   ...
   // Read and post notices for updates from the feed.  Currently assumes that
   // all items in the feed are new, coming from a WebSub hub.
   //
   // Parameters:
   // o DOMDocument $doc
   // o string $source identifier ("push")
   ...
   // Meta function that constructs an array of items for processEntries
   // harvested from an ATOM feed.
   ...
   // Meta function the parses an array of feed entries through processEntry
   // iteratively.
   ...
   // Oarameters:
   // o DOMElement $entry
   // o DOMElement $feed for context
   // o string $source identifier ("push" or "salmon")
   //
   // Returns:
   // o Notice - Notice representing the new (or existing) activity
   ...
   // Given an Entry to process, create the appropriate ActivityObject for
   // it.  Or, more specifically, trigger the events in postActiv that will 
   // process the activity.
   //
   // TODO:
   // o Make this throw an exception
   ...
   //
   // Parameters:
   // o Profile local profile of sender
   // o array in/out &$attention_uris set of URIs, will be pruned on output
   //
   // Returns:
   // o array of group IDs
   ...
   //
   // Parameters:
   // o string $profile_url
   //
   // Returns:
   // o Ostatus_profile
   //
   // Error States:
   // o throws Exception on various error conditions
   // o throws OStatusShadowException if this reference would obscure a local user/group
   ...
   //
   // Returns:
   // o mixed Ostatus_profile or null
   //
   // Error States:
   // o throws OStatusShadowException for local profiles
   ...
   //
   // Parameters:
   // o DOMElement $feedEl root element of a loaded Atom feed
   // o array $hints additional discovery information passed from higher levels
   //
   // Returns:
   // o Ostatus_profile
   //
   ...
   //
   // FIXME:
   // o Should this be marked public?
   ...
   //
   // Parameters:
   // o DOMElement $feedEl root element of a loaded RSS feed
   // o array $hints additional discovery information passed from higher levels
   //
   // Returns:
   // o Ostatus_profile
   ...
   // FIXME:
   // o Should this be marked public?
   // o Posterous code needs to be removed
   // o We should check whether this feed has elements with different <author> 
   //   or <dc:creator> elements, and... I dunno.  Do something about that.
   ...
   //
   // Returns:
   // o Avatar    The Avatar we have on disk. (seldom used)
   //
   // Error States:
   // o throws Exception in various failure cases
   //
   // FIXME:
   // o Hardcoded chmod is lame, but seems to be necessary to keep from 
   //   accidentally saving images from command-line (queues) that can't 
   //   be read from web server, which causes hard-to-notice problems 
   //   later on: http://status.net/open-source/issues/2663
   ...
   // -------------------------------------------------------------------------
   // Function: getActivityObjectAvatar
   ...
   // Fetch, or build if necessary, an Ostatus_profile for the actor in a 
   // given Activity Streams activity.  This should never return null -- you 
   // will either get an object or an exception will be thrown.
   //
   // Parameters:
   // o Activity $activity
   // o string $feeduri if we already know the canonical feed URI!
   // o string $salmonuri if we already know the salmon return channel URI
   //
   // Returns:
   // o Ostatus_profile
   ...
   // -------------------------------------------------------------------------
   // Function: ensureActivityObjectProfile
   // Fetch, or build if necessary, an Ostatus_profile for the profile in a
   // given Activity Streams object (can be subject, actor, or object).  This
   // should never return null -- you will either get an object or an
   ...
   //
   // Returns:
   // o mixed matching Ostatus_profile or false if none known
   //
   // Error States:
   // o throws ServerException if feed info invalid
   ...
   // -------------------------------------------------------------------------
   // Function: getActivityObjectProfile
   // Retrieves the URI from an ActivityObject and uses it to map out the 
   // appropriate OStatus_profile for the object.
   //
   // Parameters:
   // o ActivityObject $activity
   //
   // Returns:
   // o mixed matching Ostatus_profile or false if none known
   //
   ...
   // -------------------------------------------------------------------------
   // Function: getActivityObjectProfileURI
   // Get the identifier URI for the remote entity described by this 
   // ActivityObject. This URI is *not* guaranteed to be a resolvable 
   // HTTP/HTTPS URL.
   //
   // Parameters:
   // o ActivityObject $object
   ...
   // -------------------------------------------------------------------------
   // Function: createActivityObjectProfile
   // Create local ostatus_profile and profile/user_group entries for the 
   // provided remote user or group.  This should never return null -- you will 
   // either get an object or an exception will be thrown.
   //
   // Parameters:
   // o ActivityObject $object
   ...
   // -------------------------------------------------------------------------
   // Function: updateFromActivityObject
   ...
   // -------------------------------------------------------------------------
   // Function: getActivityObjectHomepage
   // Pretty much what it says on the tin - retrieves the stated homepage for
   // an ActivityObject.
   //
   // Parameters:
   // o ActivityObject $object
   ...
   // -------------------------------------------------------------------------
   // Function: getActivityObjectLocation
   // If the ActivityObject specifies a geolocation, then return it.  Will 
   ...
   // -------------------------------------------------------------------------
   // Function: getActivityObjectBio
   // Returns the bio for the poster of an ActivityObject if one is specified,
   // or null otherwise.
   //
   // Parameters:
   // o ActivityObject $object
   ...
   // -------------------------------------------------------------------------
   // Function: getActivityObjectNickname
   // Returns the nickname for the poster of an ActivityObject if one is specified,
   // or null otherwise.
   //
   // Parameters:
   // o ActivityObject $object
   ...
   // -------------------------------------------------------------------------
   // Function: ensureWebFinger
   // Look up, and if necessary create, an Ostatus_profile for the remote 
   // entity with the given webfinger address.  This should never return null -- 
   // you will either get an object or an exception will be thrown.
   //
   // Returns:
   // o string $addr webfinger address
   //
   // Returns:
   // o Ostatus_profile
   //
   // Error States:
   // o throws Exception on error conditions
   // o throws OStatusShadowException if this reference would obscure a local user/group
   ...
   //
   // Parameters:
   // o string $title plaintext for HTML page's title
   // o string $rendered HTML fragment for HTML page's body
   //
   // Returns:
   // o File
   ...
   // Figure out who authored an ActivityObject $activity and return their 
   // Profile.
   public function checkAuthorship(Activity $activity) {
      if ($this->isGroup() || $this->isPeopletag()) {
         ...
         if ($oprofile->isGroup() || $oprofile->isPeopletag()) {
            // Groups can't post notices in postActiv.
            common_log(LOG_WARNING, "OStatus: skipping post with group listed ".
   ...
   // -------------------------------------------------------------------------
   // Function: updateUriKeys
   // A fix for older StatusNet sites upgrading to postActiv to update the 
   // keys in the database for given profile_uri and dedupe.
